...
const geometryKey = (frame: SpectrumData) => { const f = frame.frequencyArray; return `${frame.centerFrequency}:${frame.span}:${f.length}:${f[0] ?? 0}:${f[f.length - 1] ?? 0}:${f.length > 1 ? f[1] - f[0] : 0}`; };
...
export const useSpectrumTools = (frame: SpectrumData | null, paused: boolean, enabled = true) => {
  ...
  useEffect(() => {
    ...
    const b = buffers.current; const geometry = geometryKey(frame); const x = frame.powerLevels;
    if (b.geometry && b.geometry !== geometry) buffers.current = emptyBuffers();
    ...
    for (const kind of ['power_average', 'rms_power'] as SpectrumToolKind[]) if (tools[kind].active) { const previous = state.lines[kind]; const counts = state.counts[kind] ?? x.map(() => 1); state.lines[kind] = !previous ? [...x] : x.map((v, i) => { if (!Number.isFinite(v)) return previous[i]; const n = counts[i] ?? 1; const meanPower = (10 ** (previous[i] / 10) * n + 10 ** (v / 10)) / (n + 1); counts[i] = n + 1; return 10 * Math.log10(meanPower); }); state.counts[kind] = counts; }
    ...
    if (tools.density.active) { const width = Math.min(512, x.length); const height = 128; if (state.density.length !== width * height) state.density = new Array(width * height).fill(0); state.density = state.density.map((v) => v * 0.995); x.forEach((v, i) => { const bx = Math.min(width - 1, Math.floor(i * width / x.length)); const by = Math.min(height - 1, Math.max(0, Math.floor((v + 140) / 140 * height))); state.density[by * width + bx] += 1; }); }
    ...
    setTools((current) => Object.fromEntries(kinds.map((kind) => [kind, current[kind].active && !unavailable.has(kind) ? { ...current[kind], status: 'ready', sampleCount: current[kind].sampleCount + 1 } : current[kind]])) as SpectrumToolsState);
    setRevision((value) => value + 1);
  ...
  const outputs = useMemo(() => {
    ...
    const b = buffers.current; const lineSeries: SpectrumLineSeries[] = []; const add = (kind: SpectrumToolKind, label: string, dash: number[], z: number, data = b.lines[kind], color = tools[kind].color) => { if (tools[kind].active && data) lineSeries.push({ id: `${kind}-${label}`, toolId: kind, label, frequenciesHz: frame.frequencyArray, powerLevelsDb: data, color, opacity: .9, lineWidth: 1.8, lineDash: dash, visible: tools[kind].visible, zIndex: z }); };
    add('power_average', 'Power Average', [8, 3, 2, 3], 20); add('rms_power', 'RMS Power', [2, 3], 21); add('ewma', 'EWMA', [10, 4], 22); add('min_hold', 'Min Hold', [6, 4], 29); add('max_hold', 'Max Hold', [], 30);
